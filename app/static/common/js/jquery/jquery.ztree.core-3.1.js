...
(function($) {
	...
	data = {
		...
		getTitleKey : function(setting) {
			return setting.data.key.title === "" ? setting.data.key.name
	...
	handler = {
		...
		onClickNode : function(event, node) {
			var setting = settings[event.data.treeId], clickFlag = ((setting.view.autoCancelSelected && event.ctrlKey) && data
					...
					: (setting.view.autoCancelSelected && event.ctrlKey && setting.view.selectedMulti) ? 2
							: 1;
	...
	view = {
		addNodes : function(setting, parentNode, newNodes, isSilent) {
			...
			if (parentNode) {
				//parentNode.tId = parentNode.tId.replace(".","&middot;");
				//alert(parentNode.tId)
				var target_switchObj = $(document.getElementById(parentNode.tId
						+ consts.id.SWITCH)), target_icoObj = $(document
						.getElementById(+parentNode.tId + consts.id.ICON)), target_ulObj = $(document
						.getElementById(+parentNode.tId + consts.id.UL));
		...
		appendNodes : function(setting, level, nodes, parentNode, initFlag,
			...
			var html = [], childKey = setting.data.key.children, nameKey = setting.data.key.name, titleKey = data
					.getTitleKey(setting);
			for ( var i = 0, l = nodes.length; i < l; i++) {
				var node = nodes[i], tmpPNode = (parentNode) ? parentNode
				...
				if (openFlag) {
					...
					html.push("<li id='", node.tId, "' class='level",
							node.level, "' treenode>",
							"<button type='button' hidefocus='true'",
							(node.isParent ? "" : "disabled"), " id='",
							node.tId, consts.id.SWITCH, "' title='' class='",
							view.makeNodeLineClass(setting, node),
							"' treeNode", consts.id.SWITCH, "></button>");
					...
					/*LIUBING 2012-4-13 修改*/
					var name = setting.view.nameIsHTML ? node[nameKey]
							: node[nameKey].replace(/&/g, '&amp;').replace(
									/</g, '&lt;').replace(/>/g, '&gt;');
					html.push("<button type='button' hidefocus='true' id='",
							node.tId, consts.id.SWITCH, "' title='' treeNode",
							consts.id.ICON, " class='", view.makeNodeIcoClass(
									setting, node), "' style='", view
									.makeNodeIcoStyle(setting, node),
							"'></button><span id='", node.tId, consts.id.SPAN,
							"'>", name, "</span>");
					/*end*/
		...
		appendParentULDom : function(setting, node) {
			var html = [], nObj = $(document.getElementById(node.tId)), ulObj = $(document
					.getElementById(node.tId + consts.id.UL)), childKey = setting.data.key.children, childHtml = view
					.appendNodes(setting, node.level + 1, node[childKey], node,
							false, true);
			view.makeUlHtml(setting, node, html, childHtml.join(''));
			if (!nObj.get(0) && !!node.parentTId) {
				view.appendParentULDom(setting, node.getParentNode());
				nObj = $(document.getElementById(node.tId));
			}
			if (ulObj.get(0)) {
				ulObj.remove();
			}
		...
		asyncNode : function(setting, node, isSilent, callback) {
			...
			if (node) {
				node.isAjaxing = true;
				var icoObj = $(document.getElementById(node.tId
						+ consts.id.ICON));
				icoObj.attr( {
					"style" : "",
					"class" : "ico_loading"
				...
				for (i = 0, l = setting.async.otherParam.length; i < l; i += 2) {
					if (isJson) {
						...
						tmpParam += '"'
								...
								+ ('":' + jTemp + setting.async.otherParam[i + 1])
										.replace(/'/g, '\\\'') + jTemp + ",";
					} else {
			...
			try {
				var Jtree_Ext = document.getElementById(setting.treeId).Jtree;
				var paramstr = "{\"id\":\""
						+ Jtree_Ext.Jtreeid
						+ "\",\"name\":\""
						+ Jtree_Ext.Jtreename
						+ "\",\"parent\":\""
						+ Jtree_Ext.Jtreeparent
						+ "\",\"other\":\""
						+ Jtree_Ext.Jtreeother
						+ "\",\"tableName\":\""
						+ Jtree_Ext.tableName
						+ "\",\"databaseName\":\""
						+ Jtree_Ext.databaseName
						+ "\",\"rootFilter\":\""
						+ (Jtree_Ext.param.cell.rootFilter ? Jtree_Ext.param.cell.rootFilter
								: "")
						+ "\",\"filter\":\""
						+ (Jtree_Ext.param.cell.filter ? Jtree_Ext.param.cell.filter
								: "") + "\"}";

				if (!isJson)
					tmpParam += "&params="
							+ encodeURIComponent(encodeURIComponent(paramstr))
							+ "&orderby="
							+ (Jtree_Ext.param.cell.orderby ? Jtree_Ext.param.cell.orderby
									: "");
			} catch (e) {
				//console.log(e);
			}
			...
			$.ajax( {
				...
				success : function(msg) {
					...
					if (newNodes && newNodes != "") {
						...
						view.addNodes(setting, node, tools.clone(newNodes),
								!!isSilent);
					} else {
		...
		cancelPreSelectedNode : function(setting, node) {
			...
			for ( var i = 0, j = list.length - 1; j >= i; j--) {
				if (!node || node === list[j]) {
					$(document.getElementById(list[j].tId + consts.id.A))
		...
		createNodes : function(setting, level, nodes, parentNode) {
			...
			if (!parentNode) {
				...
				var ulObj = $(document.getElementById(parentNode.tId
						+ consts.id.UL));
				if (ulObj.get(0)) {
		...
		expandCollapseNode : function(setting, node, expandFlag, animateFlag,
			...
			if (!node.open
					&& node.isParent
					&& ((!$(document.getElementById(node.tId + consts.id.UL))
							.get(0)) || (node[childKey]
							&& node[childKey].length > 0 && !$(
							document.getElementById(node[childKey][0].tId))
							.get(0)))) {
				view.appendParentULDom(setting, node);
			}
			var ulObj = $(document.getElementById(node.tId + consts.id.UL)), switchObj = $(document
					.getElementById(node.tId + consts.id.SWITCH)), icoObj = $(document
					.getElementById(node.tId + consts.id.ICON));
		...
		expandCollapseSonNode : function(setting, node, expandFlag,
			...
			if (setting.async.enable && setting.async.url
					&& setting.async.url !== '' && node) {
		...
		selectNode : function(setting, node, addFlag) {
			...
			$(document.getElementById(node.tId + consts.id.A)).addClass(
					consts.node.CURSELECTED);
			data.addSelectedNode(setting, node);
		},
		setNodeFontCss : function(setting, treeNode) {
			var aObj = $(document.getElementById(treeNode.tId + consts.id.A)), fontCss = view
		...
		setNodeLineIcos : function(setting, node) {
			if (!node)
				return;
			var switchObj = $(document.getElementById(node.tId
					+ consts.id.SWITCH)), ulObj = $(document
					.getElementById(node.tId + consts.id.UL)), icoObj = $(document
					.getElementById(node.tId + consts.id.ICON)), ulLine = view
		...
		setNodeName : function(setting, node) {
			var nameKey = setting.data.key.name, titleKey = data
					.getTitleKey(setting), nObj = $(document
					.getElementById(node.tId + consts.id.SPAN));
			nObj.empty();
			...
			if (tools.apply(setting.view.showTitle, [ setting.treeId, node ],
					...
					&& node[titleKey]) {
				var aObj = $(document.getElementById(node.tId + consts.id.A));
		...
		setNodeUrl : function(setting, node) {
			var aObj = $(document.getElementById(node.tId + consts.id.A)), url = view
					.makeNodeUrl(setting, node);
	...
	$.fn.zTree = {
		...
		init : function(obj, zSetting, zNodes, param) {
...
//			}
			...
			var zTreeTools = {
				...
				expandNode : function(node, expandFlag, sonSign, focus,
					...
					//this.reAsyncChildNodes(node,data.type,data.silent);//异步加载
					...
					if (sonSign) {
						...
						if (focus !== false) {
							$(
									document.getElementById(node.tId
											+ consts.id.ICON)).focus().blur();
						}
					}
					return expandFlag;
				},
				...
				selectNode : function(node, addFlag) {
					...
					if (tools.uCanDo(this.setting)) {
						...
						if (node.parentTId) {
							view.expandCollapseParentNode(this.setting, node
									.getParentNode(), true, false, function() {
								$(
										document.getElementById(node.tId
												+ consts.id.ICON)).focus()
										.blur();
							});
						} else {
							$(
						...
						try {
							if (setting.callback.beforeClick
									&& typeof (setting.callback.beforeClick) == "function") {
								if (setting.callback.beforeClick(
										setting.treeId, node)) {
									if (setting.callback.onClick
											&& typeof (setting.callback.onClick) == "function") {
										setting.callback.onClick(event,
												setting.treeId, node, addFlag);
									}
								}
							} else {
								if (setting.callback.onClick
				...
				updateNode : function(node, checkTypeFlag) {
					if (!node)
						return;
					var nObj = $(document.getElementById(node.tId));
