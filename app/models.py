...
class OAReceiveMail(db.Model):
    ...
    fqurey = db.Column(db.String(100), default='未查看', doc='查看状态')
...
class OASendMail(db.Model):
    ...
    ftext = db.Column(db.Text, doc='邮件内容')
    ffjid = db.Column(db.String(1024), doc='附件')
    fconsignee = db.Column(db.String(2048), doc='收件人')
    fconsigneeid = db.Column(db.String(2048), doc='收件人id')
    fstate = db.Column(db.String(100), default='未发送', doc='发送状态')
    fsendpername = db.Column(db.String(100), doc='发送人名称')
    fsendperid = db.Column(db.String(100), index=True, doc='发送人id')
